...
</br><div>base on https://github.com/oritera/Cairn</div>
---
...
系统由 **Diamond** 统筹多个 **Member**。每道题使用一个共享任务容器，Member 在各自的 `/workspace/<member>` 工作目录中探索，并可通过 `/workspace/shared` 交换产物。已确认的事实、待办意图和难度报告会写入共享黑板；Diamond 依据进度和难度按需分配更多成员，避免多个智能体在同一条路径上重复试错。找到 Flag 后，系统自动进入 Writeup、经验总结和完整性校验阶段。

支持 Web、Pwn、Reverse、Crypto、Misc、AI、OSINT 七类 CTF 题目，并提供 Playwright Browser、PyGhidra/r2 Reverse、ZAP、记忆库、工具检索等异步 MCP 服务。Web UI 可以实时查看协作图谱、时间线、日志、记忆与最终 WP。
...
- **MCP 服务**：`browser`、`reverse`、`zap` 在任务容器内运行；`memory`、`tool_search` 在应用进程内运行，`tools` 按题型动态注入。
...
- **共享能力与独立工作区并存**：同一题目的 Member 共享任务容器、附件和交换区，同时保留独立上下文与工作目录。
...
| **Reverse** | PyGhidra 真伪代码、Radare2 MCP、Objdump、Angr、Strings |
...
任务镜像还提供 Python、Java、Go、Rust、PHP、Node.js、Maven 等运行时。工具清单以 [`backend/tools/registry/`](backend/tools/registry/) 中的 YAML 注册表为准。
...
- `POST /api/platform/challenges`：随请求传入 HTTP JSON 字段映射与认证头，预览归一化题目。
- `POST /api/platform/import`：导入全部或选中的题目和附件，但不自动启动。
- `GET /api/flags`、`GET /api/flags/{project_id}`：只读查询项目 Flag 状态和平台 `external_id`。

平台认证头只在单次请求内存中使用，不写入配置或数据库。这些接口沿用项目现状不加鉴权，只应在可信隔离网络中开放 8000 端口。

### 隔离与资源控制

- 每道题默认使用一个 Docker 任务容器；Member 使用独立工作目录，共享只读约定的附件目录和 `/workspace/shared` 交换区。
- 可在 `config.yaml` 设置总 CPU、并发任务数与网络访问策略；默认最多同时运行 5 道题。
- 单任务容器不设置内存上限；停止或完成项目后回收任务容器、槽位和题目环境。
...
- [ ] **资源占用优化**：任务镜像包含 Ghidra、Chromium、SageMath、Torch 等完整工具，首次构建和磁盘占用较大。
...
| **Docker** | Docker Engine + Docker Compose v2 | 运行应用、ZAP 和每题任务沙箱 |
| **Python** | 3.10+ | 本地开发或直接运行服务时需要 |
| **LLM API** | Diamond 1 个、至少 1 个 Member 可用 | 可用 `openai`、`claudecode`、`deepseek`、`pi` 或 `mock` 格式 |
| **内存** | 按同时运行的题目规划 | 默认最多 5 个任务容器，单任务不设置 Docker 内存上限 |
| **网络与磁盘** | 首次构建需联网且建议预留 10GB+ | `ipc-task:latest` 约 8–10GB，包含 Ghidra、Chromium、SageMath 与大量 CTF 工具 |

> ⚠️ Docker 启动配置会将 Docker Socket 挂载给 `ipc-app`，以便它为题目创建任务容器。这意味着应用具备管理 Docker 的高权限；请仅在可信、隔离的开发或比赛环境使用。
...
首次构建会下载基础镜像、Ghidra/JDK 21、Playwright Chromium、语言运行时和 CTF 工具。任务镜像预计 8–10GB，耗时取决于网络和缓存。
...
本地运行 API 时同样默认使用 Docker 任务沙箱，需提前构建 `ipc-task:latest` 镜像：
...
│  共享 Task Sandbox / 独立目录 │      │ browser · reverse · zap  │
└───────────────┬──────────────┘      └─────────────────────────┘
                │
┌───────────────▼──────────────────────────────────────────────┐
...
IPC_CTFAgent/
├── backend/
│   ├── api/                 # FastAPI 路由：项目、平台、Flag、图谱、日志、WP、Memory
...
│   ├── platform/            # 通用 HTTP JSON 比赛平台适配器与字段映射
│   ├── sandbox/             # 每题 Docker 沙箱、本地沙箱、任务槽位和项目网络
│   ├── server/static/       # 单页 Web UI
│   └── tools/               # 工具注册表与 tools/tool_search MCP
├── docker/member/           # ipc-task 完整工具镜像 Dockerfile
├── docs/                    # 示例与补充文档
├── tests/                   # API、调度、MCP、Memory、沙箱测试
├── docker-compose.yml       # ipc-app、ZAP 与任务镜像编排
├── Dockerfile               # 主应用镜像
...
可用 Server 名称：`memory`、`tool_search`、`tools`、`browser`、`reverse`、`zap`。其中 `memory`、`tool_search` 与 `tools` 可传入 `--root` 指定 IPC 数据目录，`tools` 还可通过 `--category` 指定题目分类。`browser`、`reverse`、`zap` 会由 Orchestrator 通过 `docker exec -i` 在任务容器中启动；`reverse decompile` 使用 PyGhidra 返回真实 C 伪代码，失败时自动降级到 r2。
