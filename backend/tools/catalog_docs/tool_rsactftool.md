...
Automated attacks against weak RSA keys/ciphertexts.

## 用途与适用场景

RSA with suspicious modulus; try --attack all with n,e[,c].

## 版本检查

```bash
python RsaCtfTool.py --version
```

## 命令、导入与镜像路径

- 常用入口：`python RsaCtfTool.py`
- 镜像路径：`/opt/RsaCtfTool`

## 常用工作流

1. 在 Member 工作目录中确认附件、目标地址和授权范围。
2. 按题目类型使用 `python RsaCtfTool.py` 进行最小化探测。
