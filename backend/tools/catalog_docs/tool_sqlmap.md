...
Login bypass, data exfiltration, or any parameter suspected of SQL injection.
...
- 常用入口：`python sqlmap.py`
- 镜像路径：`tools/sqlmap/sqlmapproject-sqlmap-03be590`

## 常用工作流

1. 在 Member 工作目录中确认附件、目标地址和授权范围。
2. 按题目类型使用 `python sqlmap.py` 进行最小化探测。
