...
Identify front-facing WAF/CDN of a target.

## 用途与适用场景

Fingerprint defensive infrastructure.

## 版本检查

```bash
python -m whatwaf --version
```

## 命令、导入与镜像路径

- 常用入口：`python -m whatwaf`
- 镜像路径：`tools/WhatWaf`

## 常用工作流

1. 在 Member 工作目录中确认附件、目标地址和授权范围。
2. 按题目类型使用 `python -m whatwaf` 进行最小化探测。
